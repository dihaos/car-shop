...
$token = "GHVFfLuVeyxRtlCqw5nUfL4a3fysukZ.PPXPpQ4DjfAVZ";
$node = "node2";
$ct_id = "19604";
...
foreach ($exeption_numbers['exeption_numbers'] as $ex_number) {
	if ($phone == $ex_number) {
		...
		$url_vars = http_build_query([
			'unique' => 'tandem',
			'apiak' => 'GL9BtUSpERv6MPli',
			'whash' => '4a8cdb33aea6668b5ba37702a2e7f0ff',
			'calls' =>
			[
				['client' => $phone,
				'manager' => $manager,
				'site' => 'promo.gns-nissan.ru',
			]
		]
		]);
...
// Отправка заявки в callTouch START
		...
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS,
			"phoneNumber=$phone&orderComment=".$_SERVER['HTTP_USER_AGENT'].", ".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']."".($call_value != 'undefined' ? "&sessionId=".$call_value : "").($testing == true ? "&subject=TESTING" : ""));
